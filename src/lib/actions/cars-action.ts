...
import { TicketX, User } from "lucide-react";
...
export const generateImage = async (text: string, name: string) => {
  try {
    ...
    if(!privateKey) throw new Error("IMAKEKIT_PRIVATE_KEY is not defined");
...
export const addNewCar = async(carData:AddCarSchema)=>{
  ...
  await prisma.$transaction(async (tx)=>{
    ...
    const car= await tx.car.create({data:{ name,
      brand,
      type,
      year,
      mileage,
...
console.log("Car addded successfully");
...
export const getCars = cache(async ({ page = 1, type = "all" }: { page?: number; type?: string }) => {
    const limit = 8;
