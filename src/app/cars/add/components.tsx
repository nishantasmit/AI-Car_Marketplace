...
import { imagekitAuthenticator } from "@/lib/actions/imagekit";
...
export const GenerateImage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.error("Authentication failed. Could not upload image.");
    ...
    try {
      ...
      setImage(undefined); // Clear the image after successful upload
      setProgress(0);
    } catch (error) {
      ...
      {image && (
        <>
          ...
          <div className="flex justify-end gap-4 pt-4">
            <Button
              variant="outline"
              onClick={() => setImage(undefined)} // Set to undefined for consistency
            >
              Cancel
            </Button>
...
export const AddCarForm = () => {
  ...
  const onSubmitHandler = useCallback(
    async (data: AddCarSchema) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Error adding car listing";
  ...
  const autoGenerateHandler = useCallback(async () => {
    ...
    const toastId = toast.loading(`Generating details for ${carName}...`);
    setIsGenerateAILoading(true);

    try {
      const result = await autoGenerateCar(carName);
      if (!result) {
        toast.error("Failed to generate car details", { id: toastId });
        return;
      }
      ...
      toast.success("Car details generated successfully", { id: toastId });
    } catch (error) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to generate car details";
      toast.error(errorMessage, { id: toastId });
    } finally {
  ...
  // Effect 1: Sync images from Zustand store to RHF state.
  useEffect(() => {
    setValue("images", images);
  }, [images, setValue]);

  // Effect 2: Load saved data from local storage on initial mount.
  useEffect(() => {
    ...
    try {
      ...
      if (parsedDetails.colors) setColors(parsedDetails.colors);
      if (parsedDetails.features) setFeatures(parsedDetails.features);

      Object.entries(parsedDetails).forEach(([key, value]) => {
        setValue(key as keyof AddCarSchema, value);
      });
    } catch (error) {
      console.error("Failed to parse car details from local storage", error);
      localStorage.removeItem(STORAGE_KEY); // Clear corrupted data
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [setValue]); // Run only once on mount
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <button
            ...
            aria-label="Auto-generate car details"
          >
            {isGenerateAILoading ? (
...
<SelectContent>
  <SelectItem value="PETROL">Petrol</SelectItem> {/* Changed value to PETROL */}
  ...
  <SelectItem value="HYBRID">Hybrid</SelectItem>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  id="mainImage"
                  placeholder="Enter the URL of the image (direct link)"
                  value={imageUrl}
          ...
          <div className="flex flex-col gap-4 pt-4">
            <Button type="submit" disabled={submitHandlerLoading}>
              {submitHandlerLoading ? "Submitting..." : "Add Listing"}
            </Button>
          </div>
        </form>
      </CardContent>
